...
export default function EventListItem({ event }: Props) {
  ...
  // ------------------------------------------------------------------
  // ジャンル表示のためのロジックめう
 const genreString = (event.genre && Array.isArray(event.genre)) 
    // データが {genre: [{field_id: '〇〇', field_name: '〇〇'}]} のように、
    // 配列の中にオブジェクトが入っていると仮定して、.name を取得するめう！
    ? event.genre.map(g => g.name || g.genreName || '').join(' / ') 
    // もし配列じゃなくて単なる文字列ならそのまま、なければ空文字列めう
    : (event.genre || '');
  // ------------------------------------------------------------------
